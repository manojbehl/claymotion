...
public class OfferCreationProcessing {
	...
	public void actionPerformed() throws Exception{
		...
		logger.info("All Offers Fetched from hasoffer :"+ listOfOffers.size());
		...
		logger.info("Delere Already Created offers");
		...
		for (Iterator iterator = activatedAndPriorityOffers.iterator(); iterator.hasNext();) {
			...
			try{
					...
					if(offer.getPreview_url().contains("itunes.apple.com")) continue;
					
					
			
					String andriodPackage = reterievePackageName(offer.getPreview_url());
					if(andriodPackage == null || andriodPackage.trim().length() == 0) continue;
					
					offer.setAndroidPackage(andriodPackage);
					
					logger.info("Offer Package In Execution :"+ offer.getPreview_url() + "   Advertiser id :"+ offer.getAdvertiser_id() + " Name is :"+ offer.getName());


					logger.info(" Calling Generated Pixel method for :"+ andriodPackage);
					hasOfferUtility.executeHasOfferAPI(HasOfferConstant.GENERATE_PIXEL_METHOD, ""+ offer.getId());
					...
					for (Iterator iterator2 = listOfAffiliate.iterator(); iterator2.hasNext();) {
						...
						logger.info(" Generating Link for offer :"+ offer.getId() + " package :"+ andriodPackage + " and Affilate :"+ affiliate.getId() + " For Advertiser :"+ offer.getAdvertiser_id());
						try{
							jsonObject = hasOfferUtility.executeHasOfferAPI(HasOfferConstant.GENERATE_LINK_METHOD, objArray);
							...
							AffiliateOffer affiliateOffer = objectMapper.readValue(dataJsonObject.toString(), AffiliateOffer.class);
							
							
							listAffiliateOffers.add(affiliateOffer);
						}catch(Exception ex){
							logger.error(" Error  while creating Link for offer :"+ offer.getId() + " package :"+ andriodPackage + " and Affilate :"+ affiliate.getId());
...
//						clayMotionDAO.addGenerateLinkData(affiliateOffer);  
		...
		clayMotionDAO.addGenerateLinkDataList(listAffiliateOffers);
		
		setPrioirty(convertCollectionIntoList(activatedAndPriorityOffers));
		
		clayMotionDAO.updateApplicationStatus(HasOfferConstant.STATUS_PROCESSED);

		
 		
//		hasOfferUtility.executeHasOfferAPI(HasOfferConstant.FIND_ALL_METHOD, null);
		
		/*List<HasOfferData> listOfHasOfferData =  clayMotionDAO.getUnGeneratedOffer();
		
		List<Affiliate> affiliateList = clayMotionDAO.getAffiliate();
		
		for (Iterator iterator = listOfHasOfferData.iterator(); iterator
				.hasNext();) {
			HasOfferData hasOfferData = (HasOfferData) iterator.next();
			
			for (Iterator iterator2 = affiliateList.iterator(); iterator2
					.hasNext();) {
				Affiliate affiliate = (Affiliate) iterator2.next();

				if(affiliate.getId() != 317 ) continue;
				
				Object[] objArray = new Object[2];
				objArray[0] = "" +hasOfferData.getOfferId();
				objArray[1] = ""+affiliate.getId();
				
				
				hasOfferUtility.executeHasOfferAPI(HasOfferConstant.GENERATE_PIXEL_METHOD, ""+ hasOfferData.getOfferId());
				
				
				JSONObject jsonObject = hasOfferUtility.executeHasOfferAPI(HasOfferConstant.GENERATE_LINK_METHOD, objArray);
				...
				clayMotionDAO.updateGenerateLink(hasOfferData.getOfferId());
		}*/
	}
	...
	private void insertCreativeData(Collection<Offer> activatedAndPriorityOffers){
		...
		for (Iterator iterator = activatedAndPriorityOffers.iterator(); iterator
				.hasNext();) {
			Offer offer = (Offer) iterator.next();
			...
			if(androidPackage == null || androidPackage.trim().length() == 0) continue;
			...
			if(offer.getName().indexOf("_") != -1){
				String country = offer.getName().substring(offer.getName().lastIndexOf("_")+1);
				hasOfferData.setCountry(country);
				
			}
			
			hasOfferData.setName(offer.getName());
			hasOfferData.setAdvertiser(offer.getAdvertiser_id());
			...
			if(offer.getDefault_payout() != null)
				hasOfferData.setRevenueRate(Double.parseDouble(offer.getDefault_payout()));
			...
			try {
				...
				appCreative = (AppCreative)webServiceClient.executeGetMethod(creativeURL,
			...
			if(appCreative == null){
				try{
					List<AdvertiserCreative> listOfAdvertiserCreative = clayMotionDAO.getAdvertiserCreative(offer.getId());

					List<CreativeDomain> listOfCreativeDomain = dataTransformer.convertIntoCreativeDomain(listOfAdvertiserCreative, offer);

					clayMotionDAO.addCreativeData(listOfCreativeDomain);
					
					if(offer.getName().indexOf("_") != -1)
						hasOfferData.setTitle(offer.getName().substring(0, offer.getName().indexOf("_")));
					else
...
//					hasOfferData.setSubDescription(offer.get);
			...
			if(appCreative != null){
				...
				if(appCreative.getCreatives() != null && appCreative.getCreatives().size() > 0){
					try{
						List<CreativeDomain> listOfCreativeDomain = dataTransformer.convertIntoCreativeDomain(appCreative.getCreatives());
	...
	public Collection<Offer> getPriorityOffer(List<Offer> offerList){
		...
		for (Iterator iterator = offerList.iterator(); iterator.hasNext();) {
			...
			logger.info("Offer Package :"+ offer.getPreview_url() + "   Advertiser id :"+ offer.getAdvertiser_id());

			if(!doesAdvertiserPresent(listOfAdvertiser, offer.getAdvertiser_id())) continue;
			...
			if( offer.getName().lastIndexOf("_") != -1){
				countryCode = offer.getName().substring(offer.getName().lastIndexOf("_") + 1);
			}
					
			
			String keyName = offer.getPreview_url() + "_" + countryCode; 
			
			if(hashMap.get(keyName) == null){
				hashMap.put(keyName, offer);
			}else{
				Offer addedOffer = (Offer)hashMap.get(keyName);
				logger.info("Offer Added Package :"+ addedOffer.getPreview_url() + "   Advertiser id :"+ addedOffer.getAdvertiser_id());
	...
	private boolean doesAdvertiserPresent(List<Advertiser> advertisers , int advertiserId){
		for (Iterator iterator = advertisers.iterator(); iterator.hasNext();) {
			Advertiser advertiser = (Advertiser) iterator.next();
			if(advertiser.getId() == advertiserId)
				return true;
		}
	...
	private int getOfferAdvertiserPriority(List<Advertiser> listOfAdvertisers , Offer offer){
		...
		for (Iterator iterator = listOfAdvertisers.iterator(); iterator
				.hasNext();) {
			Advertiser advertiser = (Advertiser) iterator.next();
			if(advertiser.getId() ==  offer.getAdvertiser_id())
